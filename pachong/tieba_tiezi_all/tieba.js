...
var author = '';
 // 爬虫的目标地址
var url = 'https://tieba.baidu.com/p/5002575261' + '?pn=' + '1';
//  封装一层函数
function fetchPage(x){
    ...
    function startRequest(x,res){
            ...
            res.on('end' , function(){
                ...
                //  目录
                var imgDir = './tieba_image/' + tieziName + '/' ;
...
console.log("---------------------------------目录["+tieziName+"]新建完成---------------------------------");
                ...
                var allMsg = '';
        for(var i=1;i<sayContent.length+1;i++){
                var sayPeople = $('li.d_name a.j_user_card ').eq(i).text().trim();
                msg = sayContent[i-1] + '-----' + sayPeople + '-----' + txtNum + '楼';
                console.log(msg);
                allMsg = allMsg + '\r\n' +  msg;
                txtNum ++;
        }
     ...
     console.log('------------------------------文件写入"'+txt+'完成------------------------------');
      ...
      //  程序自动翻页
      webPage ++;
      var nextUrl = url.split('?pn=')[0] + '?pn=' + webPage;
      if (webPage <= page) {
                fetchPage(nextUrl);
            }
